...
- **Test health:** 2,089 TypeScript tests passing (86 files) • 108 Rust tests passing • `tsc --noEmit` clean • `cargo clippy -D warnings` clean.
...
- ✅ **Bulk migration of 18 validators to `buildCurlArgv`** (2026-04-23). Every `['curl', '-s', ...].join('\x00')` in `validator.ts` now goes through the shared builder. Same-origin validators (`sqli_error`, `sqli_blind_time`, `ssrf`, `idor`, `xxe`, `command_injection`, `path_traversal`, `cors_misconfiguration`, `host_header_injection`, `prototype_pollution`, `nosql_injection`, `bola`) inherit auth from the active hunt. Follow-redirect validators (`open_redirect`, `subdomain_takeover`, `oauth_*`) explicitly opt out — curl `-L` re-sends custom `-H` headers cross-origin, which would leak bearer/cookies to whatever host the redirect lands on. That policy is locked down by `validator_auth_pass_through.test.ts` (8 tests). Next live hunt against an auth-gated API should see `idor`, `bola`, `nosql_injection`, `command_injection`, `path_traversal`, `xxe`, `prototype_pollution`, `sqli_error` validators succeed where they previously hit 401.
- ⏳ **Deep-validator work on remaining types** — the surface is now consistent, but some validators still rely on agent self-confidence (pass-through) or narrow heuristics. Hardening pass (body-injection for sqli, canary-based prototype pollution, real OOB for ssrf) is the next increment.
